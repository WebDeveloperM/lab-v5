  ...
  useEffect(() => {
    ...
    const createFromItem = async () => {
      ...
      const itemMap: Record<string, { name: string; category: string; standard: string }> = {
        ...
        'benzin-ai-95-prisadka': { name: 'Бензин АИ-95+присадка', category: 'Бензин', standard: "O'zDSt 3031:2015" },
      ...
      try {
        ...
        // Virtual mode (id=0): the DB row is created lazily on first save rather than the
        // moment the card is clicked, so these product lines stay selectable even on a fresh
        // server whose passport_templates table doesn't have them seeded yet (see PasportaSelectPage's
        // BENZIN_STATIC/DIESEL_STATIC/etc. fallback cards, which rely on this). Jet A-1 items are
        // the one static category still excluded here — they still create a real template
        // immediately, matching PasportaSelectPage's existing-name lookup for that category.
        if (item.startsWith('diesel-') || item.startsWith('benzin-') || item.startsWith('kerosine-') || item.startsWith('mazut-') || item.startsWith('rastvoritel-') || item === 'tekhnicheskaya-sera' || item === 'szhizhennyy-gaz') {
          setDetail({
            id: 0,
            name: cfg.name,
